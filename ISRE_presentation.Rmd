...
Emotions are recognized thank to a Long short-term memory (LSTM) recurrent neural network (RNN) based on 988 acoustic features.
...
# Matching Categorical Emotion Expressed with Highest Emotion Label Recognized
...
# Matching Dimensional Emotion Expressed with Valence Recognized
...
dimension_accuracy %>% 
  ...
  ggplot(aes(emotion, avg_valence)) +
  geom_bar(stat="identity") +
  scale_x_discrete(
    name = "Emotion Expressed"
  ) +
...
# Matching Dimensional Emotion Expressed with Arousal Recognized
...
dimension_accuracy %>% 
  ...
  dplyr::mutate(emotion = forcats::fct_inorder(emotion)) %>% 
  ggplot(aes(emotion, avg_arousal)) +
  geom_bar(stat="identity") +
  scale_x_discrete(
    name = "Emotion Expressed"
  ) +
...
* The categorical recognition of emotion remains a challenge due to the diversity of affective states
